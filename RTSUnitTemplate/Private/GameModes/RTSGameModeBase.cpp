...
void ARTSGameModeBase::MulticastCreateStartMask_Implementation()
{
	...
	// Create a transient BGRA8 texture
	FogMaskTexture = UTexture2D::CreateTransient(TexSize, TexSize, PF_B8G8R8A8);
	...
	FogMaskTexture->AddToRoot();        // prevent GC
	FogMaskTexture->UpdateResource();   // alloc on GPU
...
{
	const int32 TexSize = 1024;
	const int32 BoxSize = 256;
	const int32 StartX  = (TexSize - BoxSize) / 2;
	const int32 StartY  = (TexSize - BoxSize) / 2;

	// 1) Fill CPU buffer
	for (int32 i = 0, N = FogPixels.Num(); i < N; ++i)
	{
		FogPixels[i] = FColor::Black;
	}
	for (int32 Y = 0; Y < BoxSize; ++Y)
	{
		int32 RowStart = (StartY + Y) * TexSize + StartX;
		for (int32 X = 0; X < BoxSize; ++X)
		{
			FogPixels[RowStart + X] = FColor::Red;
		}
	}

	// 2) Allocate an FUpdateTextureRegion2D *on the heap*
	...
	const uint32 SrcPitch = TexSize * sizeof(FColor);
	const uint32 SrcBpp   = sizeof(FColor);
	uint8* SrcData        = reinterpret_cast<uint8*>(FogPixels.GetData());

	// 3) Call UpdateTextureRegions with *no* bool, and let the default cleanup
	//    delete the Region for you when it's done.
...
void ARTSGameModeBase::MulticastUpdateFogMaskWithCircles_Implementation(const TArray<FMassEntityHandle>& Entities)
{
    ...
    const float InvTex   = 1.0f / TexSize;
    ...
    {
        // Map world X/Y into UV [0,1]
        float U = (WP.X - FogMinBounds.X) / (FogMaxBounds.X - FogMinBounds.X);
        float V = (WP.Y - FogMinBounds.Y) / (FogMaxBounds.Y - FogMinBounds.Y);
        // Flip V because texture Y=0 is top
        float TexXf = U * TexSize;
        float TexYf = (V) * TexSize;
    	
        int32 CenterX = FMath::Clamp(FMath::RoundToInt(TexXf), 0, TexSize-1);
        int32 CenterY = FMath::Clamp(FMath::RoundToInt(TexYf), 0, TexSize-1);
        ...
        for (int32 dY = -Radius; dY <= Radius; ++dY)
        {
            ...
            // half-width at this Y by circle eqn
            int32 HalfW = FMath::FloorToInt(FMath::Sqrt(FMath::Max(0.f, Radius*Radius - dY*dY)));
            int32 MinX = FMath::Clamp(CenterX - HalfW, 0, TexSize-1);
            int32 MaxX = FMath::Clamp(CenterX + HalfW, 0, TexSize-1);

            FColor* RowPtr = FogPixels.GetData() + Y * TexSize;
            for (int32 X = MinX; X <= MaxX; ++X)
            {
                RowPtr[X] = FColor::White;
            }
        }
    }

    // 3) Upload CPU buffer to GPU texture
    //    Allocate region on heap so the engine can free it after use:
    FUpdateTextureRegion2D* Region = new FUpdateTextureRegion2D(
        0, 0, 0, 0,
        TexSize, TexSize
    );

    uint32 SrcPitch = TexSize * sizeof(FColor);
    uint32 SrcBpp   = sizeof(FColor);
    uint8*  SrcData = reinterpret_cast<uint8*>(FogPixels.GetData());

    FogMaskTexture->UpdateTextureRegions(
        0, 1, Region, SrcPitch, SrcBpp, SrcData
    );

	/*
    // 4) Export the same CPU buffer to BMP
    const FString OutputPath = TEXT("C:/Users/info/Downloads/FogDebugOutput.bmp");
    ...
    UE_LOG(LogTemp, Warning, TEXT("Wrote fog‐mask BMP with circles to: %s"), *OutputPath);
	*/
}
	...
	UMaterialInterface* BaseMaterial, int32 MaterialIndex)
{
	if (!MeshComponent || !BaseMaterial || !FogMaskTexture)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("ApplyFogMaskMaterial: Missing MeshComponent, BaseMaterial, or FogMaskTexture"));
		return;
	}

	// 1) Assign the base material asset to that slot
	MeshComponent->SetMaterial(MaterialIndex, BaseMaterial);

	// 2) Now create a dynamic instance from that slot
	UMaterialInstanceDynamic* MID =
		MeshComponent->CreateDynamicMaterialInstance(MaterialIndex, BaseMaterial);
	if (!MID)
	{
		UE_LOG(LogTemp, Warning,
			TEXT("ApplyFogMaskMaterial: Failed to create dynamic material instance"));
		return;
	}

	// 3) Bind our runtime-updated texture
	MID->SetTextureParameterValue(TEXT("FogMaskTex"), FogMaskTexture);

	UE_LOG(LogTemp, Log,
		TEXT("ApplyFogMaskMaterial: Applied %s to %s[%d] and bound FogMaskTex"),
