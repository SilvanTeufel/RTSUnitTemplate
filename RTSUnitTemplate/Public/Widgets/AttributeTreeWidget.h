...
protected:
	...
	/** Teamnummer des zuschauenden Spielers. -1, wenn sie noch nicht vergeben ist. */
	int32 MyTeamId() const;

	/** Der GameState, der die Teamtoepfe fuehrt. Auch auf dem Client vorhanden. */
	const class AUpgradeGameState* TeamGameState() const;
	...
	void HandleReset();
	/** Schliessen-Knopf: zurueck auf die Startansicht (siehe ShowStartScreen). */
	void HandleClose();

	UPROPERTY()
	ALevelUnit* TargetUnit = nullptr;

	// ENTFERNT am 20.09.2026: AktuellesZiel, GefundenesZiel und VertreterProTag.
	//
	// Das Widget zeigt den Baum DES TEAMS und braucht dafuer keine Einheit. Die beiden
	// Zwischenspeicher waren nur noetig, weil je Knoten eine Vertretereinheit gesucht wurde -
	// und GefundenesZiel wurde nie verworfen, was die Anzeige dauerhaft auf einer Einheit
	// festnagelte, auf der gar nicht gebucht wurde.
